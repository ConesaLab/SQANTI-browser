...
- 📊 **Per-category tracks** for easy exploration (main track in pack mode by default)
- 🔎 **Trix search** with underscore format for finding isoforms by attribute
- 📋 **Interactive HTML tables** with export and Trix string generation
...
By default, the **main SQANTI3 track** opens visible in **pack mode**. **Validation tracks** (STAR junctions, CAGE peaks, PolyA peaks, reference)—when included—also show by default. **Category-specific tracks** (e.g., full-splice_match, novel_in_catalog) start **hidden** so the view stays uncluttered. You can turn on individual category tracks from the track list as needed.
...
- Optional `validation_tracks` section: `CAGE_peaks`, `polyA_peaks`, `star_sj`, `reference` (colors embedded in bigBed so they persist when moving to custom track)
- When used, custom colors apply to **tracks, track HTML pages, and interactive table reports** (including SVG diagrams)
...
**Validation track colors** (embedded in bigBed; persist when moving to custom track):

| Track | Default | SVG name |
|-------|---------|----------|
| CAGE_peaks (TSS) | Green #008000 | green |
| polyA_peaks (TTS) | Dark red #C80000 | darkred |
| star_sj | Blue #1565C0 | dodgerblue |
| reference | Gray #464646 | dimgray |
